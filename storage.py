...
transaction and lock the task row before touching its attempts.  Claims use
``FOR UPDATE SKIP LOCKED`` so concurrent workers never wait on or share a task.
"""
...
def create_task(sender_id: str, recipient_id: str, input_text: str, idempotency_key: str | None) -> dict[str, str]:
    with write_transaction() as db:
        ...
        if idempotency_key is not None:
            # Serialize requests sharing (sender, key) so the lookup and insert
            # below act as one operation even when two API processes race.
            db.execute(select(func.pg_advisory_xact_lock(func.hashtext(f"{sender_id}:{idempotency_key}"))))
            existing = db.scalar(
