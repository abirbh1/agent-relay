...
Status: v1 starter. This document defines the protocol and behavior implemented by Agent Relay on PostgreSQL.

The original starter used SQLite; storage now runs on PostgreSQL. The HTTP protocol, credential rules, task lifecycle, and delivery guarantees are unchanged by the port.
...
- **PostgreSQL:** persists agents, tasks, and delivery attempts. Transactions with row locking (`FOR UPDATE SKIP LOCKED` for claims and recovery) coordinate concurrent claims across API/worker processes.
